...
#### 🚨 **시스템 에러 로깅 대시보드 (Admin전용 Logs 페이지)**
- **서버 에러 실시간 캡처**: 백엔드에서 예외 발생 시 `logToDb` 유틸리티를 통해 에러 메시지, 발생 위치(source), 스택 트레이스를 `app-log`에 즉시 기록합니다.
- **오토 리프레시 및 필터링**: 최신 상태를 유지하는 토글 방식의 자동 새로고침(Auto-refresh) 기능 지원. Fatal / Error / Warn 레벨 필터링 및 키워드 검색을 지원합니다.
- **Top 5 에러 소스 추적**: 가장 빈번하게 오류가 발생하는 코드 위치(Source)를 상단에 집계하여 빠른 대처가 가능합니다.
- **UI/UX 최적화**: KST(한국 표준시) 기반의 크로스플랫폼 호환 날짜 포맷 제공. 낡은 로그는 관리자가 버튼 하나로 7일/30일 단위로 일괄 삭제(정리)할 수 있습니다.
- **보안 라우팅**: 기본 권한(Core Router) 충돌을 방지하기 위해 커스텀 라우팅(`auth: false`) 및 별도 컨트롤러로 안전하게 연동되며, 프론트엔드는 전용 `/admin/login` 인증을 통과해야만 접근할 수 있습니다.
