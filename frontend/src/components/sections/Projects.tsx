...
export function Projects({ projects, featured = false }: ProjectsProps) {
  if (!projects || projects.length === 0) {
    return (
      <motion.section className="text-center" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }} viewport={{ once: false, amount: 0.2 }}>
